    ...
    return (
        <div className="w-full overflow-hidden bg-gray-900 border-b border-gray-800">
            <div className="flex items-center px-4 py-2 space-x-3 min-w-max">
                <div className="flex items-center px-3 py-1.5 text-sm text-gray-300 bg-gray-800/50 rounded-lg">
                ...
                {trendingStocks.map((stock, index) => (
                    <div
                        key={stock.id}
                        className="flex items-center bg-gray-800/40 hover:bg-gray-800/60 cursor-pointer transition-all duration-200 rounded-lg px-3 py-1.5 group"
                    >
                        <div className="flex items-center space-x-2">
                            <span className="text-gray-500 text-xs font-medium">#{index + 1}</span>
                            <div className="flex items-center">
                                <span className="text-white font-medium">{stock.symbol}</span>
                                <span className="ml-1.5 text-xs px-1.5 py-0.5 bg-gray-700/50 text-gray-300 rounded group-hover:bg-gray-700">
                                    {stock.emoji}
                                </span>
                            </div>
                        </div>
